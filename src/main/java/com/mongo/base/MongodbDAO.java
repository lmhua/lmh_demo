...
public abstract class MongodbDAO<T>
{
	...
	public Pagination<T> getPage(int pageNo, int pageSize, Query query)
	{
		...
		System.out.println("query执行：" + query);
	...
	{
		WriteResult writeResult = this.mongoTemplate.upsert(query, update, this.getEntityClass());
		return writeResult;
	}
	 ...
	 * @param collection
	 */
	public void remove(T bean, String collection)
	{
		this.mongoTemplate.remove(bean, collection);
	}
